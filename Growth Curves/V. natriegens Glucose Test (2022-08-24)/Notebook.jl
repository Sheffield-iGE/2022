...
# ╔═╡ 0ec74e46-e8f4-4ea7-8cca-bbf1f3372713
# Isolate the H media
hlogdf = filter(:Replicate => r -> r == "H", pdf)

# ╔═╡ 4a2de0f5-c40a-4297-b18f-c2087eb67ff1
...
begin
	...
	md"The doubling time for H was ~$(round(g, sigdigits=3)) minutes, with a growth rate of $(round(coef(hols)[2], sigdigits=3))"
end

# ╔═╡ 8c07376e-2e6e-4185-80f0-162e37cc9aa8
# Isolate the S media
slogdf = filter(:Replicate => r -> r == "S", pdf)

# ╔═╡ 088cd337-5d82-42e9-9543-d72af5e00f3b
# Replot, but the isolated values so that we can pick out the exponential growth region
...
# ╔═╡ 46814097-a959-40fa-b0a6-ecdef86c935f
# Trim the data to take a closer look at log-phase
tslogdf = filter(:Time => t -> 20 <= t <= 82, slogdf)

# ╔═╡ ce3c3a39-b138-48a5-9e4f-89586ecc2237
# Log-transform the OD data
transform!(tslogdf, :OD => ByRow(log2) => :OD)

# ╔═╡ da03efe4-85a1-4613-8c0e-4a5813239e94
# Perform and ordinary least-squares regression for a linear model
sols = lm(@formula(OD ~ Time), tslogdf)

# ╔═╡ 41a3b75b-81e2-4b57-93c6-0c9b559573aa
# Insert a new column into our dataframe representing the model predictions
tslogdf[!,:Model] = predict(sols)

# ╔═╡ efa00d6c-2bd2-4efd-87e1-733af9e69a4e
# And then plot it on a log-scale
...
# ╔═╡ 4dfc8526-219a-4209-b8ac-61e963528a25
begin
	# Calculate doubling-time
	sg = 1/coef(sols)[2]
	# Format it into a nice string
	md"The doubling time for S was ~$(round(sg, sigdigits=3)) minutes, with a growth rate of $(round(coef(sols)[2], sigdigits=3))"
end

# ╔═╡ 4a172f9e-0f6b-4e15-abc5-5736408ff828
# Isolate the no glucose media
nglogdf = filter(:Replicate => r -> r in ["1", "2", "3"], pdf)

# ╔═╡ 496bcd08-d656-481b-bee1-a2bef5373751
# Replot, but the isolated values so that we can pick out the exponential growth region
...
# ╔═╡ 75098cdc-a38f-485d-8f48-1b711a5e98bd
# Perform and ordinary least-squares regression for a linear model
ngols = lm(@formula(OD ~ Time), tnglogdf)

# ╔═╡ 042d49e8-86c7-4f18-8960-07d5e6598b94
# Insert a new column into our dataframe representing the model predictions
tnglogdf[!,:Model] = predict(ngols)

# ╔═╡ 4d199f9f-0e46-4a36-9619-ea72708c105a
# And then plot it on a log-scale
...
# ╔═╡ a310d59f-f64d-4b94-8124-de7b1e838384
begin
	# Calculate doubling-time
	ngg = 1/coef(ngols)[2]
	# Format it into a nice string
	md"The doubling time for no glucose was ~$(round(ngg, sigdigits=3)) minutes, with a growth rate of $(round(coef(ngols)[2], sigdigits=3))"
end

# ╔═╡ 625ee40c-c810-461b-8240-0afbbdfbd527
# Isolate the glucose media
glogdf = filter(:Replicate => r -> r in ["4", "5", "6"], pdf)

# ╔═╡ 1808e050-085e-412d-bfde-0f65764f887f
# Replot, but the isolated values so that we can pick out the exponential growth region
...
# ╔═╡ 689333e0-4013-4ee7-a95c-2fdca9567e07
# Perform and ordinary least-squares regression for a linear model
gols = lm(@formula(OD ~ Time), tglogdf)

# ╔═╡ c3ac1d30-eeca-4875-a96d-55e56a7a7f7c
# Insert a new column into our dataframe representing the model predictions
tglogdf[!,:Model] = predict(gols)

# ╔═╡ 93aa1b80-33d9-4e54-880c-800e5df4bfd1
# And then plot it on a log-scale
...
# ╔═╡ 13aa5ec2-e1c5-46a5-85d0-4e0ad7dca0b9
begin
	# Calculate doubling-time
	gg = 1/coef(gols)[2]
	# Format it into a nice string
	md"The doubling time for no glucose was ~$(round(gg, sigdigits=3)) minutes, with a growth rate of $(round(coef(gols)[2], sigdigits=3))"
end

# ╔═╡ 2a2fac4d-b045-4ab5-b8c7-6cce29115d23
# Isolate the with / without glucose media
comparedf = filter(:Replicate => r -> isdigit(r[1]) , pdf)

# ╔═╡ 2b759a61-db0d-4d27-aeb7-37043ed40dc0
# Replot, but the isolated values so that we can pick out the exponential growth region
