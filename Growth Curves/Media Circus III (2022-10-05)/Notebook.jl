...
begin
    ...
    df = stack(df, 2:7, variable_name=:Replicate, value_name=:OD) |> dropmissing
end
...
# ╔═╡ 7c6cafce-f950-45de-bb1e-d1a154a9188b
# Get the e2YT data
e2ytdf = filter(:Replicate => r -> 1 <= parse(Int, r) <= 3, pdf)

# ╔═╡ ec08487b-369d-404c-ba3b-099c348243dd
# Get the DoE data
doedf = filter(:Replicate => r -> 4 <= parse(Int, r) <= 6, pdf)

# ╔═╡ 4622387e-3574-4666-a04c-e740f82055ae
# Group e2YT data by time-point
e2ytgdf = groupby(e2ytdf, :Time)

# ╔═╡ 214f39e6-b7f9-4f95-8f6f-d3ff6449e77a
# Average all of the e2YT replicates
e2ytfdf = combine(e2ytgdf, :OD => mean => :OD, :OD => (o -> max(0, mean(o) - 1.96 * std(o) / √length(o))) => :CIL, :OD => (o -> mean(o) + 1.96 * std(o) / √length(o)) => :CIH)

# ╔═╡ 574fdb8f-f303-458e-8032-e2f3baf0851a
# Trim the data to take a closer look at log-phase
e2ytcdf = filter(:Time => t -> 100 <= t <= 180, e2ytfdf)

# ╔═╡ 2336a8d9-ccbb-49f5-8c0e-6e11cb66409f
# Log-transform the OD data
loge2ytcdf = transform(e2ytcdf, :OD => ByRow(log2) => :OD, :CIL => ByRow(log2) => :CIL, :CIH => ByRow(log2) => :CIH)

# ╔═╡ 8080c6da-094e-41e7-bbd4-b1b22cb00bb9
# Perform and ordinary least-squares regression for a linear model
e2ytols = lm(@formula(OD ~ Time), loge2ytcdf)

# ╔═╡ 60e72978-49e7-4773-bba1-a6c28be99eeb
# Insert a new column into our dataframe representing the model predictions
loge2ytcdf[!,:Model] = predict(e2ytols)

# ╔═╡ abf42aad-85fc-4946-9dd3-0a81b41e9270
# And then plot it on a log-scale
plot(loge2ytcdf, x=:Time, y=:OD, Geom.point,
	 Guide.xlabel("Minutes"), Guide.ylabel("OD₆₀₀"),
	 Guide.title("<i>E. Coli</i> Growth Curves"),
	 # With the line-of-best-fit superimpoosed
	 layer(x=:Time, y=:Model, Geom.line),
     # With the error-bars superimposed
	 layer(x=:Time, ymin=:CIL, ymax=:CIH, Geom.yerrorbar))

# ╔═╡ 1ea23b8d-36a9-4599-b549-3eb92cb815d1
# Group DoE data by time-point
doegdf = groupby(doedf, :Time)

# ╔═╡ 06bb4f39-b024-4829-a89f-f903ccc54456
# Average all of the DoE replicates
doefdf = combine(doegdf, :OD => mean => :OD, :OD => (o -> max(0, mean(o) - 1.96 * std(o) / √length(o))) => :CIL, :OD => (o -> mean(o) + 1.96 * std(o) / √length(o)) => :CIH)

# ╔═╡ 2214356c-26fb-4f66-a72b-d1faa98bad1c
# Trim the data to take a closer look at log-phase
doecdf = filter(:Time => t -> 100 <= t <= 180, doefdf)

# ╔═╡ e4acae41-a230-4c60-ad3c-17aa93977ae9
# Log-transform the OD data
logdoecdf = transform(doecdf, :OD => ByRow(log2) => :OD, :CIL => ByRow(log2) => :CIL, :CIH => ByRow(log2) => :CIH)

# ╔═╡ 1d6782d3-e7fc-4703-b60f-877bba415f72
# Perform and ordinary least-squares regression for a linear model
doeols = lm(@formula(OD ~ Time), logdoecdf)

# ╔═╡ 71130c4c-9fff-48bd-babf-15f6e93acf67
# Insert a new column into our dataframe representing the model predictions
logdoecdf[!,:Model] = predict(doeols)

# ╔═╡ 4d52eb79-a15b-4dfb-8ef0-e9e39830bd04
# And then plot it on a log-scale
plot(logdoecdf, x=:Time, y=:OD, Geom.point,
	 Guide.xlabel("Minutes"), Guide.ylabel("OD₆₀₀"),
	 Guide.title("<i>E. Coli</i> Growth Curves"),
	 # With the line-of-best-fit superimpoosed
	 layer(x=:Time, y=:Model, Geom.line),
...
begin
	...
	e2ytg = 1/coef(e2ytols)[2]
	# Format it into a nice string
	md"The e2YT doubling time was ~$(round(e2ytg, sigdigits=3)) minutes, and the growth rate was $(round(coef(e2ytols)[2], sigdigits=3))"
end

# ╔═╡ 3a18cddc-5e5c-4451-ab98-e21003d9e603
begin
	# Calculate doubling-time
	doeg = 1/coef(doeols)[2]
	# Format it into a nice string
	md"The DoE doubling time was ~$(round(doeg, sigdigits=3)) minutes, and the growth rate was $(round(coef(doeols)[2], sigdigits=3))"
end
